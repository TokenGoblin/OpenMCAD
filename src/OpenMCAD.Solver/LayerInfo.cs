namespace OpenMCAD.Solver;

/// <summary>
/// Marks the <c>OpenMCAD.Solver</c> layer.
/// </summary>
/// <remarks>
/// <para>
/// ISketchSolver and IAssemblySolver, DOF analysis, and constraint diagnostics
/// (ADR-0006). Sketch semantics live above this; only the numerical contract lives here.
/// </para>
/// <para>
/// This type exists so the assembly is never empty and so tests/arch has a stable anchor to
/// resolve the layer by. It carries no behaviour. Substantive work on this layer begins at
/// P4-T02.
/// </para>
/// </remarks>
public static class LayerInfo
{
    /// <summary>Gets the short name of this layer, as used in PLAN.md 4.1.</summary>
    public static string Name => "Solver";
}
