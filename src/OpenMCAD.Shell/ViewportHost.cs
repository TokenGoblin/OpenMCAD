...
public sealed class ViewportHost : HwndHost
{
    ...
    public ViewportHost(ILogger<ViewportHost>? logger = null)
    {
    ...
    {
        // Order matters. The swapchain references the queue, so it goes before the device; the
        // device waits for the GPU before releasing anything, so the window can only go last.
        _target?.Dispose();
        _target = null;
