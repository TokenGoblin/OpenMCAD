...
- [ ] **P3-T18** Persistence: OPC container writer/reader, MessagePack document schema v1, geometry and tessellation cache blobs, thumbnails, manifest. *(In progress. The MessagePack primitives are done and conformance-tested; the schema, the container and the caches are not. The encoder is hand-written rather than taken from a package -- a decision taken deliberately because `Directory.Packages.props` requires an ADR for any new dependency, and because the one thing a library would have given, attribute-driven serialisation, is unusable here: P3-T20 requires unknown fields to survive a round trip, which means explicit read and write code either way, so the library would only have supplied the primitives. The encoding is canonical -- always the shortest form that holds a value -- and that is a correctness requirement rather than tidiness, because the first exit criterion is a bit-identical re-save and the bytes therefore have to be a function of the value alone. The expected byte sequences in the tests come from the format specification rather than from this code's own output, which would only prove it is self-consistent. `ReadRaw` captures a value without interpreting it, which is what P3-T20 will preserve unknown fields with.)*
