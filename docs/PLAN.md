...
**What is left: the document type itself.** `Assembly` is a structure nothing holds yet -- wiring it into `Document` means a slot, transaction operations, a codec section and `Matches`, none of which is hard but all of which is a separate concern from getting §5.9's distinction right. It also runs into a question worth not answering alone: how a document says which *kind* it is. P5-T07 adds a third kind, and deciding the shape with two of the three in view is guessing at the third.)*
