...
**What is left: the numerical solve.** `IAssemblySolver` has no implementation, which is the same state `ISketchSolver` was in before `FakeSolver`. It wants residuals per mate kind and a least-squares step over seven parameters per body -- a quaternion, a translation, and a normalisation residual to keep the quaternion honest -- and it is what turns the lower bound above into a real rank-based diagnosis with `Redundant` and `OverConstrained` sets.)*
