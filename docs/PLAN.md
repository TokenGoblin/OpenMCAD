...
- [ ] **P2-T01** `IRenderDevice` thin RHI abstraction; D3D12 implementation via Vortice: device, swapchain, descriptor heaps, fences, upload ring, frame pacing. *(Device, descriptor heaps, fences and the upload ring are done and tested against a real D3D12 device with the debug layer on — WARP, so they are covered on a build agent with no display rather than only on a developer's desk. `IRenderDevice` stays thin per ADR-0008: heaps and rings are D3D12 concepts and are not on it. The swapchain and frame pacing wait on P2-T02, because both need a window to present to. Two bugs in the upload ring's occupancy logic were caught by its tests — a ring filled exactly to capacity reported itself empty — and it now has a randomised test as well as worked examples.)*
- [ ] **P2-T02** `HwndHost`-based viewport control; per-monitor DPI v2; resize, occlusion, device-lost recovery. *(`SwapChainTarget` is done and tested against a real off-screen Win32 window — flip-model with three buffers, resize that releases and rebuilds, a minimised window ignored rather than failed, presenting rotating through every buffer, and device loss returned rather than thrown because it is a normal event. The DPI arithmetic lives in `OpenMCAD.Render` rather than the WPF host so it can be tested at all, ADR-0014 keeping the Windows target framework in the shell. `ViewportHost` itself compiles but is **not** verified: it needs a message loop, a monitor and a person looking at it. Occlusion is not started.)*
...
- [ ] **P2-T04** Tessellation pipeline: kernel triangulation → GPU buffers, double→float relative to a per-view origin, per-face ID attribution, LOD levels. *(Partly done: `SnapshotBuilder` does the double→float conversion against a sticky render origin and the per-face ID attribution, with tests. Rounding the origin to a grid turned out not to be enough on its own — a scene centred on a grid line flips between two origins on a millimetre edit and re-uploads every buffer — so the origin is carried forward until the scene genuinely drifts from it. GPU buffers wait on P2-T01; LOD is not started. Edge polylines now arrive from the kernel; see P2-T06.)*
- [ ] **P2-T05** Shaded face pass with instancing and GPU frustum culling.
- [ ] **P2-T06** Edge rendering: depth-biased polylines, screen-space width, silhouette edges for curved surfaces. *(The kernel half is done: `triangulate` now returns edge polylines taken from the polygons OCCT fits to the face triangulation, so an edge lies exactly on the surface it bounds rather than standing off it by the chordal deviation. Three IDL operations, an explicit `ProducesEdgeTessellation` capability so the contract battery demands edges only from a kernel that claims them, and a test that every edge point coincides with a mesh vertex. Depth bias, screen-space width and silhouettes are GPU work and wait on P2-T01.)*
- [ ] **P2-T07** ID pass into R32_UINT; readback path with a staged, latency-hiding queue; face/edge/vertex resolution with an edge/vertex proximity bias so thin entities are pickable.
- [ ] **P2-T08** Camera and navigation: orbit, pan, zoom, zoom-to-fit, zoom-to-selection, standard views, view cube, perspective/orthographic toggle, configurable mouse profiles. *(Orbit, pan, zoom, zoom-to-fit, standard views and the projection toggle are done, with `Mat4d.LookAt`/`PerspectiveFieldOfView`/`Orthographic` added to the maths library. All arithmetic, so all of it is tested — 37 tests, and the two that matter most were verified by breaking the camera deliberately to confirm they fail. Yaw is applied about world up and pitch about the camera's right, so roll cannot accumulate; fitting uses the bounding sphere against whichever of the two field-of-view axes binds, so a wide model on a wide viewport is not clipped and orbiting does not resize it; orthographic zoom changes the view volume rather than the distance, because dollying a parallel projection does nothing; near and far come from the scene, because perspective depth precision is governed by their ratio. The view cube and configurable mouse profiles are not started — the first needs rendering, the second is input mapping in `OpenMCAD.Interaction`.)*
- [ ] **P2-T09** Selection and highlight rendering: hover, selected, pre-selected, error states; selection sets in `OpenMCAD.Interaction`.
- [ ] **P2-T10** Weighted-blended OIT for transparency.
...
- [ ] **P2-T15** `AssemblyLoadContext` plugin loader + a hello-world sample plugin that adds a ribbon button. *(Loader done, with a real sample plugin — and three more fixture assemblies, one per rejection path, because a plugin declared inside the test assembly would share the host's types by construction and prove nothing about isolation. The shared-contract set is narrow on purpose: `OpenMCAD.Api` resolves from the host so that the plugin's `IPlugin` is the host's `IPlugin`, which is what avoids "unable to cast object of type IPlugin to type IPlugin". A plugin built against another API version is refused before `Initialize` runs, and one that throws is reported rather than allowed to stop the application starting. The ribbon button is not done: it needs the shell's UI.)*
...
- [ ] **P3-T01** `Document`, `Feature`, `Body`, `Parameter`, `ReferenceGeometry` core types; `FeatureId`/`BodyId` as GUID-backed strong IDs.
- [ ] **P3-T02** `IDocumentTransaction`: open/mutate/commit/rollback; internal setters so nothing mutates outside a transaction.
- [ ] **P3-T03** Dependency DAG construction from declared feature inputs; cycle detection with a named cycle in the error.
- [ ] **P3-T04** `RebuildEngine`: dirty marking, transitive propagation, topological ordering, execution against the dispatcher, cancellation, coalescing of superseded rebuilds.
- [ ] **P3-T05** Geometry cache keyed by `(FeatureId, hash(resolved inputs))`; eviction policy; `--no-cache` mode.
- [ ] **P3-T06** Rollback bar as DAG-prefix evaluation.
- [ ] **P3-T07** Error containment: per-feature error state, `suppressed-by-error` propagation, rebuild report object.
- [ ] **P3-T08** **`PersistentName` and `NameSegment`** per §5.3 — structure, serialization, human-readable rendering for diagnostics.
- [ ] **P3-T09** Name resolution tier 1: history replay over `HistoryMap` chains.
