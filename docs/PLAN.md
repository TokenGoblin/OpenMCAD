...
**What is left: a drag session.** `MateSystem.For(..., moving:)` is the shape a drag needs, and P4-T07's sketch drag is the precedent for what is still missing around it -- coalescing moves inside a frame budget, solving only the group `MateAnalysis.Groups` says is affected rather than the whole product, and a minimal-motion objective so a dragged assembly does not rearrange itself more than it must.)*
