...
- [ ] **P2-T04** Tessellation pipeline: kernel triangulation → GPU buffers, double→float relative to a per-view origin, per-face ID attribution, LOD levels. *(Partly done: `SnapshotBuilder` does the double→float conversion against a sticky render origin and the per-face ID attribution, with tests. Rounding the origin to a grid turned out not to be enough on its own — a scene centred on a grid line flips between two origins on a millimetre edit and re-uploads every buffer — so the origin is carried forward until the scene genuinely drifts from it. GPU buffers wait on P2-T01; LOD is not started. Edge polylines need a kernel operation that does not exist: `triangulate` returns faces only, and a CAD edge must not be recovered from the triangle mesh.)*
