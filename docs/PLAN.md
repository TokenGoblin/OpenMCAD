...
- [x] **P2-T10** Weighted-blended OIT for transparency. *(Sorting back to front fails on exactly what CAD produces — a housing containing its own contents, two parts interpenetrating, a body whose faces overlap from the current angle. Sorting is per object and the failure is per pixel, so no ordering of objects fixes an object that overlaps itself; the symptom is faces popping in front of one another during an orbit, which reads as the model changing. Weighted blending accumulates with a depth-dependent weight and keeps the product of what each fragment let through — both commutative, so order cannot matter. It is an approximation, but a uniform and stable one, which beats occasional exactness that flips as the camera moves. The test that matters renders two overlapping bodies both ways round and requires the same pixel, having first checked both actually reach it. Opacity is a single figure for the scene because a `DisplaySnapshot` carries geometry and identity but no appearance; per-body materials were expected to arrive with the document model, and did not -- Phase 3 landed and `Body` carries no appearance, so they are P6-T08's.)*
- [x] **P2-T11** Grid, origin triad, reference plane display, background gradient/environment. *(Reference planes added later; the rest below. **Reference planes (later).** The blocker recorded here was P2-T10's transparency, which landed eight commits *after* this task and went unnoticed -- `docs/notes/open-decisions.md` records the pattern, since the same thing happened to P3-T13. `DisplayPlane` carries a frame and no size, because a plane is conceptually infinite and the square standing for it is sized from the scene at draw time: storing an extent would freeze it at whatever the scene was when the snapshot was built, and taking it from the camera would hold the planes still on screen while the model grew against them -- the same reasoning as the grid's spacing. It carries no colour either, because a snapshot holds geometry and identity and no appearance (P2-T10); the tint is a `PlaneStyle` on the pass, beside `EdgeStyle` and `AxisStyle`. Depth-tested and not written: tested because a datum belongs in the scene and should be hidden by the solid standing on it, not written because three datums meeting at the origin overlap across most of the region they share and the first one drawn would otherwise hide the other two outright. Deliberately *not* routed through P2-T10's weighted blending, and the reason is stated in the pass rather than assumed: that machinery buys freedom from ordering where fragments genuinely occlude one another, and three flat quads through the origin are not that -- the visible cost of an arbitrary order is a slightly different tint where two cross, against two full-size targets and a composite. One real bug, caught by the pixel test that reads the middle of a plane back rather than by reasoning: `BlendDescription.AlphaBlend` is *premultiplied* despite the name, so a shader returning straight colour draws a datum solid -- `EdgePass` already carried that note and `Axes.hlsl` already multiplies through, and this shader now does too. Two of my own test expectations were wrong before that: the front view looks along -Y, so the plane square-on to the camera is XZ and not XY. The grid is computed per pixel from a ray-plane intersection rather than drawn as lines: line geometry needs an extent and a spacing fixed in advance, and a CAD user zooms across six orders of magnitude in a session. Spacing snaps to a power of ten below a tenth of the scene and is taken from the scene rather than the camera, because a reference that changes density as you zoom is worse than one at the wrong scale. The triad is depth-tested and the gizmo is not, which is the difference between a landmark and an overlay — the eye reads 'drawn over a solid' as 'in front of the solid' and no colour choice argues it out of that.)*
...
**Sketch topology change (later).** The seventh category, and the recorded blocker for it was wrong: it was never Phase 4. The naming layer had always had the seam -- `HistoryNameResolver` takes a `Func<NameSource.Sketch, SubEntity>` and turns a name whose provenance bottoms out in a sketch entity into either the entity, `NotFound` if the line is gone, or `Unsupported` if nothing can answer -- but `RebuildEngine` never passed one, so every sketch-sourced name resolved `Unsupported` in a real rebuild whatever the document said. One optional constructor parameter, threaded to the resolver, is the whole fix. Three scenarios: adding a line leaves a reference to another line's wall alone (the reference is anchored to the sketch entity, not to a role, which is the point of the category -- a role-and-ordinal reference could not survive the sketch gaining a wall in the middle of its list); removing the named line breaks that reference and *only* that one, with a repair, rather than quietly adopting a neighbouring side wall, which is the silent corruption §5.3 forbids and which both neighbours would have looked like a reasonable answer for; and a host with no lookup reports `Unsupported` rather than missing, asserted on the outcome rather than on the wording. The sketch is a `Feature.Settings` entry, so editing it is an ordinary document edit that dirties the feature -- which is how the cache bug below was found.

**A real bug in P3-T05's cache key, found on the way.** `Feature.Settings` was not in `RebuildKey` at all. Settings are, in P3-T21's own words, what a feature is told that is not a dimension and not a selection -- which direction, how many, which end condition, whether to merge -- and every one of those changes the geometry, so a feature whose only edit was a setting hit the cache and got back what it looked like beforehand. Exactly the class of bug the version tag's own comment describes for entity references at version 2, and invisible in the same way. Now hashed at version 3, sorted by name and by ordinal rather than culture (an `ImmutableDictionary` promises no enumeration order, and hashing it as it comes would give one feature different keys in different processes -- ADR-0011, presenting as a cache that mysteriously stops hitting), each value tagged with its kind (untagged, text and a choice encode identically and would share an entry while meaning different things). The pinned `AKeyIsTheSameInAnyProcess` value was recomputed the way its own comment demands -- a second implementation of the encoding in another language, validated by reproducing the old version-2 value before being trusted for version 3 -- and the feature it pins now carries one setting of every kind, so each tag is pinned rather than merely written down. Eight sabotages, all caught: the engine seam, the settings block, the count, the ordering, and each of the five kind tags. Two earlier attempts at the kind-tag test were too weak to catch a single dropped tag and were replaced rather than kept -- a number against a flag does not collide, and text against a choice only collides when *both* tags go.)*
...
**Projecting a circle onto a plane it is not parallel to (later).** One of the three things this task deferred, and the one with real geometry in it: an orthographic projection is linear, so a circle written as `C + r·cos θ·u + r·sin θ·v` lands as `c + cos θ·A + sin θ·B` — already an ellipse, but not in the form `SketchEllipse` stores, because `A` and `B` are in general neither perpendicular nor of equal length. Finding the axes is one closed-form step: `|p(θ) − c|²` is a sinusoid in `2θ`, so its extremes are half a turn apart in `2θ` and a quarter turn apart in `θ`, which is why one `atan2` gives both axes rather than only the longer. The reason for doing it this way rather than fitting the projected points is that **the eccentric angle then falls out for free**: `A cos θ + B sin θ` is identically `M cos(θ − θ₀) + N sin(θ − θ₀)`, so a point at circle-angle θ is at eccentric angle θ − θ₀ exactly. `SketchEllipticalArc` stores eccentric angles (P4-T03) precisely so that evaluating a point needs no transcendental solve, and recovering them from the projected geometry would put that solve straight back in. The handedness correction is the third appearance of the same rule: a projected frame that comes out clockwise describes the same physical arc traversed the other way, and since `SketchEllipticalArc` sweeps anticlockwise from a minor axis a quarter turn ahead of its major, the ends swap as well as the angles negating — arrived at independently here and agreeing with what P4-T03 and the antiparallel circular edge already needed.

Eight sabotages, and the first run caught **two**. That was the useful part: every fixture I had written put the circle's own reference direction along the axis the tilt turns about, which makes the projected pair perpendicular already, sends `θ₀` to zero, and leaves the halving, the axis construction and the eccentric-angle shift all doing nothing observable. A skewed fixture — reference direction deliberately not the tilt axis — plus a property test that every point of the circle, projected, satisfies the ellipse equation of the ellipse produced, takes it to five of six. Two of my own test expectations were wrong and the code right: one asserted a centre as a literal `(1, 2)` when a sketch plane's axes come from `Plane.CreateFrame` and are not the world's, so it was asserting the test's idea of the frame; the other expected the two views of one arc to swap ends, when the correct answer is that they produce the *same* entity, since a sketch arc describes ground and the ground is the same. The sixth sabotage found dead code rather than a weak test: the swap that put the longer axis in `major` could never run, because `atan2(2A·B, |A|²−|B|²)` names the maximum by construction — confirmed over two hundred thousand random pairs before deleting it, since a branch no input can reach is a branch nothing can check. The last one is honest rather than caught: `SketchEllipse.Degeneracy` would refuse an edge-on projection anyway, so the explicit check changes no outcome. It stays because the reason it gives names the view — 'seen edge-on' — rather than the ellipse, and it is written as an angle rather than a measurement of what came out, since a length threshold on a radius refuses a perfectly reasonable aspect ratio for a millimetre-scale hole. Still deferred: conic and spline edges, and `Intersect` against a circular edge, which needs a multiplicity policy this does not have.)*
- [x] **P4-T12** Sketch dimensions: linear, aligned, angular, radial, diametric, ordinate; placement, display, and editing. *(Most of "aligned", "radial" and "diametric" already existed -- P4-T04's `Distance`, `Radius` and `Diameter` constraints measure exactly those, driving or reference, and needed nothing new. What was actually missing were the two pieces the constraint model alone cannot answer: a distinct "linear" measurement, and where a dimension's line and text actually go. `ConstraintKind.HorizontalDistance`/`VerticalDistance` are the first -- a linear dimension measures one axis regardless of which way the two points actually lie from each other, which is a different equation from `Distance`'s hypotenuse whenever they are not already axis-aligned, not merely a different display of the same number. Both are unsigned, the same convention `Distance` already uses: which point a user clicked first is an accident of drawing order, not a claim about which side of the other it sits on. `SketchDimension` (a placement: which constraint, where the witness point is) and `SketchDimensionLayout` (witness lines, dimension line and text position, resolved fresh from the current geometry every time) are the second, split the same way `SketchPlaneReference`/`SketchPlane` are in `OpenMCAD.Modeling` (P4-T10): a placement choice is durable, where the line ends up today is not. `SketchDimensionLayout` reads its displayed value from the live geometry rather than from `SketchConstraint.Value`, which matters for a reference dimension (a driving one's value and its geometry agree only once the solver has actually run since the value last changed) and is what "display" asks for -- the current truth, not the target. "Editing" needed no new mechanism: a driving dimension's value is `SketchConstraint.Value`, already changeable via `ConstraintSet.With` (P4-T04), and a value edit is a rebuild like any other. Incomplete as first built, and marked so at the time: layout covered only the point-to-point kinds (`Distance`, `HorizontalDistance`, `VerticalDistance`) -- `Distance`'s point-to-line operand shape, `Angle`, `Radius` and `Diameter` each need real, separate geometry (an arc radius picked from the witness point for an angle; a leader direction and an inside-or-outside decision for a radius or diameter) that three-sevenths of a general solution would not have been better than not building. Ordinate dimensioning is not a fourth layout at all -- the number it shows against a shared baseline is exactly what `HorizontalDistance` or `VerticalDistance` already measures from that baseline point, and only the presentation (one shared extension line, several dimension lines stacked to avoid colliding text) differs; that stacking is a layout problem across several dimensions at once, which a single dimension's signature has nowhere to put, and is left for when several exist together rather than half-built now. Two sabotages, both caught while writing the tests rather than needing to run them wrong first: an early version checked "do both operands resolve to a point" before checking the constraint's kind, which reported `Angle` and `Radius` -- whose operands name whole entities via `EntityPoint.Self` and so never resolve to a point at all -- as their geometry having vanished rather than as the unsupported kinds they are; fixed by dispatching on kind first. Two more sabotages by reverting an actual guard: the entity-exists check that tells a deleted point apart from `Distance`'s unsupported point-to-line shape, and the coincident-points check in the aligned layout, which without it does not throw -- it silently returns NaN geometry as `Resolved`.

**The remaining four layouts (later).** Every dimension type §5.6 names is laid out now: angular, radial, diametric, and `Distance`'s point-to-line operand shape. Each needed one decision that the geometry alone cannot make, and in every case it is the witness point that makes it -- which is the pattern the three point-to-point kinds already had, arrived at again rather than invented. A point-to-line distance turned out not to be a fourth layout at all: once the foot of the perpendicular is found it is an aligned dimension between two points, so it goes through the same code, and the foot comes from the *infinite* line rather than the segment, or the dimension would depend on how far the line happens to have been drawn rather than on where it points. An angular dimension has to choose one of the four angles at a crossing, and the text sits in exactly one of them; the arc's radius is how far the text is from the vertex; and each line gets a witness line only when it stops short of the arc, since one already running past it needs no extension and a zero-length one is a stray tick in the picture. A radial or diametric dimension has to choose a leader direction and whether the leader sits inside the circle or outside it, and dragging the text past the rim is how a user says 'outside'. On an arc the leader is brought back onto the arc's own sweep rather than refused -- the user asked for this arc's radius, and the nearest place the arc actually is remains a true answer to that, where a leader aimed past the end would point at empty space. `DimensionLayout` gained a second, nullable shape for its line, because an angular dimension's is an arc: forcing it into a pair of points loses the bulge that is the whole shape of one, and making every consumer handle a general polyline would charge the six kinds that are a straight line for the one that is not. Exactly one of the two is set when resolved. Ordinate dimensioning is still not a fourth layout for the reason already recorded -- only its stacking across several dimensions at once is left, and a single dimension's signature has nowhere to put that. Ten sabotages, nine caught at once. The tenth found a real gap rather than a nuisance: both angular tests happened to sweep anticlockwise from the first line's ray, so the branch that swaps the two when the second ray sits clockwise of the first was never exercised -- emitting them in operand order names the same pair and sweeps the other three quarters of the turn, which reads as a plausible arc pointing the wrong way. A case with the text in the fourth quadrant now catches it. One of my own tests was wrong and the code right, in the same area and for the same reason: it asserted the arc ran from the ray pointing down round to the one pointing along -X, which is the long way.)*
- [x] **P4-T13** Sketch editing tools: trim, extend, offset, mirror, linear/circular sketch pattern, fillet/chamfer corner, split, convert to construction, scale, move, rotate, copy. *(Seven of the eleven tools are, underneath, the same operation: apply one `SketchTransform` to a selection. Move, rotate and scale edit the selected entities in place; copy, mirror and the two patterns instead add a transformed *copy*, keeping the original -- which is not a detail, it is the actual difference between "drag this" and "place another one of these", and the two behaviours share `SketchGeometryTransform` (maps one entity's geometry through a transform) rather than one deciding what the other means. `SketchTransform` is the sketch-plane counterpart of `OpenMCAD.Math.Transform`: scale, optional reflection, rotation, translation, in that order, with reflection kept as a flag rather than a negative scale so `ScaleAbout` can reject a nonsensical factor instead of quietly also mirroring. `MirrorAbout` rests on the standard identity that reflection about a line through the origin at angle θ is `Rotate(2θ) ∘ Flip` -- checked against three independent axis-aligned cases before trusting it for the general one. `Duplicate` -- what copy, mirror and each pattern instance all call -- keeps a copied selection's own internal constraints (remapped onto the new entities, when every entity a constraint names is in the copied set) and drops the rest: duplicating "concentric with that fixed hole" onto every pattern instance would point all of them at the one fixed hole, which is a contradiction the moment there is more than one instance, not a pattern. `LinearPattern`/`CircularPattern`'s `count` is the total number of instances including the original, matching what a pattern dialog actually asks for, and the circular one spaces instances at `totalAngle / count` rather than `/ (count − 1)`, so a full-circle bolt-circle pattern lands evenly rather than leaving a gap where an unrequested closing instance would sit. An arc under a transform that reflects needs its Start and End swapped to stay describable at all -- `SketchArc` can only ever sweep anticlockwise (P4-T03), a reflection reverses which way increasing angle turns, and the identical conclusion P4-T11 already reached for a circular edge viewed from the far side of a sketch plane is reused here rather than re-derived. Scoped to point, line, circle and arc; ellipse, elliptical arc, parabola and hyperbola each need their own angle handling worked out with that same care and report `Unsupported` rather than silently wrong geometry, the same choice P4-T11 made for a curve kind it does not project. Offset, fillet and chamfer are no part of *this* group -- each is a curve-topology operation (building and re-trimming a parallel curve at every corner, inserting a blend whose neighbours' endpoints move to meet it) with essentially nothing in common with "map a transform over a selection", and each arrived later as its own thing (`SketchCorner`, `SketchOffset`; see below). Five sabotages, each failing the test that names the exact property it removed: the doubled-angle identity in `MirrorAbout` (caught by three of five mirror tests at once), the Start/End swap on a reflected arc, `Duplicate`'s all-operands-copied requirement (weakened to any, which throws trying to remap an entity never copied -- itself a finding, since the honest failure here is louder than a silently wrong constraint would have been), and an off-by-one in each pattern's loop bound.
...
`SketchSplit` is the tenth, scoped to line and arc -- deliberately not circle, which has no second end for a single cut to produce two pieces from (cutting a closed loop at one point gives one open curve, not two; `SketchTrim` already covers turning a circle into an arc, but that needs two points, not one). It exists as `SketchTrim`'s counterpart precisely because it does not share trim's unresolved question: nothing is deleted, so every point of the original entity provably belongs to exactly one of the two results, and the four cases follow from that rather than from a judgement call -- `Start` needs no change, `End` is remapped onto the new piece since it is no longer where the first piece ends, `Self` and (for an arc) `Centre` are facts about the whole original entity that stay true of both halves and so are kept on the first piece and duplicated onto the second, and `Middle` -- the midpoint of the *original* entity, not of either piece afterwards -- has no such rule and refuses the whole split with `ConstraintNotTransferable` rather than silently measuring to the wrong place. Four sabotages, each caught by reverting the exact rule it depended on: dropping the `Middle` check entirely (the split then proceeds and leaves the constraint pointing at a place that no longer means what it did), skipping the `Self`/`Centre` duplication (both the line and the arc case caught this at once), and skipping the `End` remap.

`SketchCorner` is the eleventh and twelfth together, and they are one operation rather than two: fillet and chamfer differ only in what is dropped into the corner once the corner is known, and everything before that -- where the two lines' infinite extensions meet, which side of that point each click asked to keep, which endpoint therefore gives way, whether the leg is long enough to give up what the blend needs -- is identical. Writing them apart would have been two chances to disagree about which corner was pointed at, which matters because two lines that cross rather than merely meet make four corners and only the clicks tell them apart. Scoped to line--line, where the geometry is exact and closed-form (tangent points at `r / tan(theta/2)` from the corner, centre at `r / sin(theta/2)` along the bisector, no case analysis at all); a leg that is an arc puts the blend's centre at an intersection of *offset* curves, which exists in up to four places depending on whether the blend sits inside or outside each arc, and choosing among them is a judgement a click alone does not settle -- so `Unsupported`, the same call `SketchExtend` made about arcs. A leg is extended to its tangent point when it stops short of the corner and trimmed back past it when it overshoots, both falling out of one rule (the end nearer the corner along the surviving direction is the one the blend takes over) rather than being two cases. The constraint story is the part with a real decision in it: the corner's own `Coincident` is *removed*, and that is the operation rather than a side effect, since after a blend the two ends demonstrably are not in the same place and keeping it would leave a sketch contradicting itself at the next solve -- the two coincidences onto the blend's own ends replace it, and a fillet adds a tangency to each leg as well, which is what keeps the corner a corner under a drag instead of three pieces of geometry that happen to touch today. Any *other* constraint naming a moving end -- a `Fix` on the corner, a distance measured to it -- refuses the whole thing with `ConstraintNotTransferable`, on `SketchSplit`'s principle that a constraint which quietly starts measuring to the wrong place is worse than an operation that declines; a constraint naming a whole leg is untouched, a shortened line being no less horizontal than a long one. The four constraints a fillet adds against the arc's five parameters look redundant by counting and are not: the coincidences pin only the ends, a circle through two given points is still a one-parameter family, one tangency uses that up and the other is a fact about the far end nothing has yet said. Thirteen sabotages across two rounds, each caught by the test naming the property it removed -- inverting the tangent length (which a right angle cannot catch, since `tan` and its reciprocal agree there, and the 135-degree case exists for exactly that reason), taking the major arc, moving the wrong end, leaving the stale corner coincidence, dropping the fit check, ignoring the click, making a blend construction when either leg is. One found a thin spot rather than a bug: solving the intersection against the wrong direction was caught by only a single test, because nearly every fixture put the corner at both lines' `Start`, where the formula returns the right answer for free -- so a case with the corner on neither leg's start, and the second leg's `End` giving way, was added and now catches it too. Offset is the one tool of the eleven still outstanding.

`SketchOffset` is the last of them, and the reason it went last is that offsetting one curve is arithmetic while offsetting a chain is the actual operation: a line moves sideways and an arc changes radius, but the offsets of two curves that met at a corner do not meet -- short of one another where the corner turns away, overshooting where it turns towards -- and closing every one of those gaps is the work. Joins are mitred, taken out to where the two offsets really cross, because that is what makes the offset of a chain of *n* pieces a chain of *n* pieces; rounding them with inserted arcs is the other standard answer and a real one, but it adds geometry the selection has no counterpart for. Three decisions were not obvious. First, the distance is **signed** rather than taken from a click, unlike every other tool here: a click says which side of one curve was meant, but a chain has to be offset to the same side all the way along or its pieces will not join, so the side is a property of the chain and turning a cursor into that sign is one cross product belonging to whatever draws the preview. Positive is left of travel, which makes a circle's inside its left, since an arc runs anticlockwise (P4-T03) -- so a positive offset of a circle is the smaller one, falling out of the rule rather than being a rule of its own. Second, the chain is **discovered**, not taken on the caller's word: the entities are ordered by their endpoints and anything that is not a single open chain or single closed loop is refused, because a wrong selection producing plausible geometry joined in the wrong sequence is far worse than a refusal. Third -- and this one was a bug found by a test, not by reasoning -- discovering the order leaves the *direction* open, a chain being walkable from either end, and direction is what 'left' means. The first version let it fall out of whichever loose end the walk happened to reach first, which is an accident of the order the caller listed things in: the same selection, reordered, silently offset to the other side. It now runs through the entity named first in that entity's own Start-to-End direction, which a caller can see the effect of and a UI can use directly -- the entity clicked first decides the side. Alone among the editing tools this one only ever adds, so the question that took the most care in `SketchSplit` and `SketchCorner` -- which constraints follow what -- simply does not arise; the new pieces get a coincidence at each join for the same reason a blend does. A piece consumed by its own two joins is refused, and is detected by how far each end moved rather than by what is left, because a collapsed arc and one sweeping nearly the whole way round are indistinguishable after the fact -- a sweep is only ever reported as a positive angle. An offset running into *another* part of the same chain, which is what makes offsetting genuinely hard, is not caught and is not claimed to be. Nine sabotages. Five were caught immediately; four survived, and every one of them was a real hole rather than a nuisance -- no test had a collinear join (where the two offsets are the same infinite line and have no crossing to be taken out to, so the already-met check is what stops an ordinary chain failing), none had an arc collapsing rather than a line, and the branch test was being caught by the loose-end count rather than by the degree check it was written for, which a closed square with a diagonal across it -- no free ends anywhere -- now exercises properly. Three tests were added and those three are caught. The fourth is left standing and said so in the source: taking the crossing nearest the loose ends rather than nearest the original corner is the better-motivated rule, but no case has been found where the two differ while the result is still sensible, so it is not claimed as more than that.)*
...
The eleven editing tools of P4-T13 are wired in now, which was the gap this task's own note and `docs/specs/sketch.md` both named. One method each, and one private helper doing the only thing they all share: six tools have six result types with one shape between them -- did it work, what came out, why not -- and giving those six records a common interface for one caller's convenience would be the tail wagging the dog, since what each adds beyond that shape (both pieces of a split, a blend's id, every piece an offset made) differs and none of it is a view model's business. The selection is passed **in order**, and that is load-bearing rather than tidy: `SketchOffset` takes its direction from the entity named first, so sorting it here would decide which side of a chain the user's offset landed on. Twelve tests and seven sabotages. Four were caught at once; three survived and each was worth the finding. One was a genuinely weak assertion of mine -- 'every mutation re-solves' checked only that `LastSolve` was non-null, which an earlier `AddLine` had already made true, so deleting the re-solve entirely passed; it now asserts the solve is a *different* one, which is the difference between a fresh readout and a stale one. The other two were claims in my own comments that the code does not need: the `!resolved` half of the guard is unreachable while every tool's failure carries no sketch, and the `Distinct` on the selection is redundant because every tool already deduplicates (`SketchEdit.Duplicate` says so in its own remarks). Both stay -- the alternative is depending on coupling nobody has written down -- but they now say they are belt and braces rather than implying a test proves them.)*
...
- [x] **P5-T03** Datum planes, axes, and points as named, resolvable reference geometry. *(Split the way P4-T10 and P4-T11 are: `DatumDefinition` is the durable half -- thirteen constructions, each naming its inputs by `DatumReference` -- and `DatumResolver` turns one into a `ReferenceGeometry` for a single rebuild. Started ahead of the rest of Phase 5 deliberately: everything else here waits on `OPENMCAD_WITH_OCCT` (open-decisions #3), and this is the one task that does not, because a datum is naming and arithmetic and needs only the three geometry queries P4-T10 and P4-T11 already take as delegates. **Storing the resolved plane instead is the mistake this exists to avoid**: a datum that recorded coordinates would stop following what it was built on the first time anything upstream moved, and then there was no reason to build one rather than type the numbers. `Owner` is not on the definition, which does not know which feature hosts it until it is placed -- but `Name` is, because `(Owner, Name)` is the pair `Document.FindReference` looks up by and a name arriving from anywhere but the file would move when that anywhere did. `DatumReference` names *what is pointed at* -- reference geometry by name, or kernel topology by `PersistentName` -- and not what it is for; a datum has up to three inputs in three geometric roles, and the same input kind serves different roles in different constructions (a face is a plane to offset from, a vertex a point to run an axis through), so enumerating source-by-role the way `SketchPlaneReference` can afford to would be a combinatorial table of cases that all resolve identically. Asking the role at resolution time keeps one table instead of nine. Four failure outcomes, split where the *repair* differs rather than where the cause does: `NotFound`, `Ambiguous` and `NotUsable` all mean the reference is wrong and are fixed by pointing at something else, `Degenerate` means every reference was fine and the geometry has no answer. Two constructions needed a decision the geometry alone cannot make. A plane at an angle demands the axis **lie in** the plane -- both halves, perpendicular to the normal *and* passing through it -- because an axis merely parallel to the plane rotates to an offset copy at an angle of zero, which is the answer disagreeing with itself; that "no rotation reproduces the source" property is what the test asserts, rather than a coordinate. And an axis-meets-plane intersection treats an edge as the infinite line it names even when the edge stops short, which is the opposite of what a sketch's `Intersect` does with the same input (P4-T11) -- that one is bringing real geometry into a profile, this one is a construction, and an axis is by definition unbounded. Deliberately not built: a plane tangent to a cylinder, a plane normal to a curve at a point, and an axis down a cylindrical face, all of which need a surface query no layer here has. Twenty sabotages, each failing exactly the test that named the property removed and, in nineteen cases, nothing else -- dropping the circle's `PerpendicularTo` correctly failed two, since the reference-direction test and the degenerate-circle test are both about that line.*
**The feature half (later).** Thirteen feature types rather than three with a "method" to choose between, because `PropertyCondition` decides visibility on one property having one value and every construction shares some inputs with some others -- a single `DatumPlane` type could not say "this input applies when the method is offset *or* angle". Generalising the condition would be extending the schema for one feature's convenience, and the result would be worse: a file now records which construction was used in `FeatureType`, the one field never dropped or defaulted. Grouping thirteen back into three menus is `Category`'s job and the ribbon's. The datum's name is the feature's name, since a type that makes exactly one datum has exactly one thing to call it and a second name in a setting could disagree with the tree.

Building it needed two additions to the document model, and the first is a hole that predates this task. **`PropertyKind.Selection` could be declared in a schema and never read back**: position in `Feature.References` was the only link to the property it satisfied, and position stops being a link the moment an input is optional or can be answered another way. `EntityReference` now carries the input it satisfies, defaulted to empty and written only when present, so a file re-saved by this build does not grow an empty string in every reference (P3-T18's exit criterion is that a re-save is bit-identical). A datum plane through three points is the first feature where getting two inputs the wrong way round produces geometry rather than an error. The second addition is `ReferenceValue`, a feature's pointer at reference geometry: a value rather than an `EntityReference` because reference geometry has a name it keeps and is found in one step, where topology has to be traced through history and carries a multiplicity policy because the face it named can become several. Topology stays in `Feature.References` regardless -- that is where `FeatureGraph` reads its edges -- so `PropertyKind.Reference` is an input satisfied either way, and a file naming both is refused rather than resolved by preferring one.

`DatumResolver` stopped taking a `NameResolver` and now takes what a reference already came to. The rebuild engine resolves every declared reference before an evaluator runs, so resolving them again inside the resolver would be the same work done twice and could come out differently -- the geometric tier scores candidates against a model, and the model is not in the same state at both moments. `DatumResolver.Through` adapts a resolver for the callers that genuinely have a name and no answer yet.

`DatumFeatureEvaluator` is the first real `IFeatureEvaluator`, and it touches no kernel, which is why it could be first. One difference from everything below it: **it throws where `DatumResolver` returns data.** The resolver's other caller is an interactive tool that must show the user what went wrong while they are still choosing; a rebuild's containment boundary is `RebuildEngine`'s catch, which records `FeatureState.Failed` against that one feature. Returning an empty output there would report success and quietly delete the datum.

Nineteen sabotages. The end-to-end test through the real engine earned its place immediately: `RebuildKey` refuses to hash a `FeatureValue` kind it does not know -- deliberately, since hashing only a type name would make two different values look identical -- so `ReferenceValue` would have round-tripped through the codec and then thrown at the first rebuild. Three sabotages survived the first pass and each was a real gap. The anonymous-reference test compared two documents' byte lengths, which a codec writing the field unconditionally still satisfies because an empty string is shorter than a real one; it asserts on the encoded map now, the same trap `FeatureSettingTests` already records for the settings sort. `Check`'s `Reference` case was unreachable, because nothing declared such a property with a default -- a test that a schema *can* declare one, and that only a pointer will do, makes it live rather than deleting it. And a missing dimension was silently substituted with zero, which would have built a plane coincident with the one it was meant to be offset from and reported success; the schema catches that first, but `FeatureSchema` lives in `OpenMCAD.Modeling` and the rebuild engine does not run it, so the translation's own check is the last line of defence and now has a test.)*
- [x] **P5-T04** Assembly document type: component definitions vs. occurrences (§5.9), occurrence tree, transforms. *(`OpenMCAD.Core/Assemblies/`. The structure and the transforms are done; the document type is not, and the split is deliberate -- see the end. Unblocked for the same reason P5-T03 was: an assembly is structure and arithmetic, so none of it waits on `OPENMCAD_WITH_OCCT`. **§5.9 is more emphatic about this one distinction than about anything else in the section** -- "never duplicate the definition. This distinction is structural — get it wrong and large assemblies become unusable" -- so `ComponentDefinition` (what a component *is*, one entry per document however often it appears) and `ComponentOccurrence` (where one instance *sits*) are separate types with separate ids, and the test that makes the sentence checkable places one bolt ten thousand times and asserts there is still one definition. The consequence that earns the split is that renaming the part is one edit rather than ten thousand, which is why `Name` lives on the definition and `Label` on the occurrence. A definition holds a `Source` string rather than a loaded document, because §5.9's Lightweight and Graphics-only modes exist precisely so a large assembly need not open everything it names; `ComponentKind` is recorded rather than discovered for the same reason, since a tree, an instance count and a cycle check would otherwise all require loading every document. That means a file can be wrong about it, which is a cross-document check for P5-T11 and a cheaper mistake than the alternative.

**An instance has no id, and that is the design working rather than a gap.** A sub-assembly placed twice is one document with one set of occurrences reached through two parents, so its children carry the same `OccurrenceId` under both -- minting fresh ids per placement would be exactly the duplication §5.9 forbids, wearing a different hat, and would break the moment someone added a bolt to the sub-assembly. `OccurrencePath` is the chain from the top down, and it is the shortest thing that can name an instance: ids are unique within each assembly, so the chain is unique within the product. `IsUnder` is a prefix test rather than an "is this id in the path" test for the same reason -- the same sub-assembly placed elsewhere shares its children's ids, and hiding one must not hide the other.

`WorldTransformOf` composes outermost-first and takes a loader delegate, the same shape `DatumResolver`'s geometry queries take and for the same reason: the layer that can open another document is above this one. Not caching a world transform per instance is a decision rather than an omission -- there is no instance to cache it on, and the cache would have to be invalidated for every instance under a sub-assembly whenever that sub-assembly moved, which is the ten-thousand-item update the whole arrangement exists to avoid. Hidden and suppressed are separate flags, deliberately: a suppressed occurrence is not in the product and has no mass, a hidden one is merely not drawn, and one flag would make "let me see past this cover" silently change the assembly's mass. A per-occurrence configuration selection is listed in §5.9 and is *not* here -- configurations are Phase 14, and a slot with no rules about a missing configuration would be a promise this build cannot keep.

Twenty-one sabotages, twenty caught by exactly the intended test. The twenty-first is honest rather than caught: replacing the path's content hash with one that depends only on its length fails nothing, and should not -- equal paths still hash equally, which is the entire contract, and a weaker hash is a performance question rather than a correctness one. The equality trap this codebase has now met four times (an `ImmutableArray` compares by reference) is caught, by a test that puts a path in a `HashSet` and looks for it with an equal one built separately.

**The document type (later).** One document type carrying a `DocumentKind`, not three types. Everything the container does -- transactions, undo, parameters, metadata, the unknown-field preservation that lets a newer file open -- is identical for all three, and the alternative is that machinery written once and then inherited or copied twice; what actually differs is which payload is populated. The earlier worry here, that deciding the shape with only two of the three kinds in view would be guessing at the third, turned out to be answered already: **`Serialization.DocumentPackage` had been declaring its own `DocumentKind` with exactly these three values since P3-T18**, because a package manifest has to say what it holds. So there were two vocabularies for one concept and nothing forcing them to agree. The enum moved to `Documents`, where the concept belongs, and the manifest now reads the document's own answer. Values keep their original order, so no file already written changes meaning.

The kind is **recorded rather than inferred**: a document with no occurrences could be a part or an empty assembly, and deriving it from what is present would turn an assembly into a part the moment its last component was deleted. A part refuses `SetAssembly` outright, because one that could quietly acquire occurrences would be an assembly nobody had declared. `Drawing` is declared now although nothing produces one until P5-T07 -- the cost of the value existing early is nothing, and the cost of adding it after a release is a migration for every file already written.

The transaction takes the **whole structure** rather than offering an operation per component. `Assembly` is already immutable and already refuses a placement of a component it does not have, so a set of `AddOccurrence`/`RemoveOccurrence` methods would give those rules a second home -- and §5.9's warning is precisely about what happens when the definition/occurrence distinction is enforced in two places that disagree. It is still one transaction and therefore one undo. Nothing marks a feature dirty: an assembly's structure is not produced by the feature graph, and what a placement affects is where geometry sits in the world, which is P5-T06's live mate solving rather than a rebuild.

A part writes **no assembly section at all** rather than an empty one, since `persistence.md` §3 wants the bytes to be a function of the document and an empty section in every part file would change all of them to say nothing. A file written before documents had a kind reads as a part, which is what every one of them is.

Fourteen sabotages. Two survived the first pass and were the same finding twice: the test for `Matches` added a component *and* placed it, so removing either the definitions check or the occurrences check failed nothing, because the other still saw the difference. Two narrower tests fix it -- a component listed but not placed, and the same component placed by one document and not the other -- and each now fails exactly its own check. The full build then caught something no sabotage would have: a CLI test asserting "the document is a seven-field map" before appending a field from the future, which had quietly become a second record of how many fields a document carries. It derives the count now, since what it means is "one more than there are".)*
- [x] **P5-T05** `IAssemblySolver` and the minimal mate set: coincident, concentric, distance; grounded components; DOF reporting. *(`OpenMCAD.Solver/Assemblies/`. The model, the interface, the diagnosis vocabulary and the freedom analysis are done; the numerical solve is not. Split the way the sketch side was split across P4-T02, P4-T05 and P4-T06 -- this task bundles all three of those jobs for assemblies, and doing the model first is what let the analysis be written and tested before any solver exists.

**The solver is below `OpenMCAD.Core`, so it cannot name an occurrence**, and that constraint shaped everything. `MateBodyId` is the solver's own handle, exactly as `SketchEntityId` is, and mapping an `OccurrencePath` onto one is the modelling layer's job at P5-T06. What comes out is a rigid-body constraint solver that knows about bodies, placements and residuals and nothing about documents or files -- which is also what lets it be handed a sub-assembly, a flattened product, or a fixture with three cubes. `MateElement` is the same idea one level down: a user mates a face to a face, and what constrains the arithmetic is the plane that face lies on, so topology is reduced to a plane, an axis or a point *before* it reaches here. Elements are held in the body's own frame rather than the world's, because a mate says something that stays true wherever the assembly is dragged; a world-space element would have to be rewritten every iteration.

**`MatePairing` is a table of decisions, not derivations**, and the tests state them so that changing one is deliberate: planar coincident removes 3 (two slides and a spin survive), concentric axis-axis removes 4 (a slide and a spin survive), point-to-point removes 3 (a point has no orientation to align, which is why it is not 6), and a point distance removes 1 -- a sphere of positions, not a point. A distance mate between planes removes the same 3 as a coincident one, because a distance mate *is* a coincident mate with an offset. Combinations are refused rather than approximated: concentric between two points says nothing, and a distance to an axis is ambiguous until somebody decides whether it means the nearest approach or the parallel offset -- the same call `DatumResolver` makes about constructions it will not attempt, and every refusal carries a reason, since §5.6 is explicit that a diagnosis without one leaves the user deleting things at random.

**The freedom count is deliberately a lower bound and named as one.** Six per ungrounded body, less what each mate removes on its own, which is exact only when no two mates say the same thing. Telling redundancy apart needs the rank of the Jacobian, which needs the solve -- so `MateFreedom.AtLeast`, `CouldBeFullyConstrained` and `MayBeOverConstrained` say what they can prove and no more. A status bar reading "0 degrees of freedom" on an assembly that can still be dragged is worse than one reading "at least 0", because the first is a claim a user acts on. `Groups` is P4-T05's union-find again, with one difference that matters: a grounded body joins no group, or two components mated to the same fixed base would be merged and every drag would re-solve the whole product.

Twenty sabotages, and the tests found two things before them. The first run failed `AMateFromABodyToItselfRemovesNothing` -- the doc comment on `AssemblyMate.Bodies` said a self-mate constrains nothing a solve can move, and the code counted it anyway, so the prose was right and the implementation wrong. Two sabotages then survived: the grounded-pair half of that same guard was covered only incidentally, and `MateElement.PlacedBy` was tested only through a plane, so breaking the point or axis case failed nothing. Both now have tests of their own.

**The numerical solve (later).** `MateResiduals` in `OpenMCAD.Solver` says what each mate *means* -- separate from any solver, the way `ConstraintResiduals` is -- and `FakeAssemblySolver` in `OpenMCAD.Solver.Fake` drives them to zero by Levenberg-Marquardt over a numerical Jacobian, reusing the `Dense` factorisation `FakeSolver` already needed. Same bargain as its sketch counterpart: correct, dependency-free, not fast. Unlike the sketch side there is no planegcs waiting to replace it, so if it proves quick enough it may simply stay.

**Seven numbers per body rather than six**, because three rotation angles have gimbal lock, where two of the three stop being independent and the Jacobian silently loses a column. A quaternion has no such orientation, at the price of one redundant number paid for by one residual per body saying its length is one. Scale is not solved at all -- mates say where a component sits, not how big it is. Residuals are deliberately more numerous than the freedom they remove (two planes flush are four numbers describing three), because the alternative is choosing a basis from the geometry and any such choice degenerates for some orientation.

**Two things the tests found before any sabotage did, and both were design faults rather than slips.**

The first: *plane-plane mates did not converge at all*, in the commonest case there is. Two faces both pointing up, asked to be flush, sit exactly on a saddle of the least-squares objective -- turning a unit vector at the pole changes its component along the pole only to second order, so the entire residual lies in a direction the Jacobian cannot see and `JT r` is zero. Every gradient method stops dead there. The fix is to nudge off the stall and try again, which also *tells a saddle from a genuine contradiction*: a contradiction stalls again from the new position. The turn is a fixed axis and angle rather than a random restart, because ADR-0011 asks a rebuild to be reproducible.

The second: *dependent rows are not redundant mates*. Reading the rank's dependent rows as redundancy flagged a fully-mated body as `Redundant`, and would have flagged nearly every mate ever written -- three point pins are nine rows describing six degrees of freedom, and those rows are dependent by construction. A mate is redundant when **taking it away leaves the rank unchanged**, which is a rank per mate over a matrix already computed. Conflicts are not a rank question at all: the mates in conflict are simply the ones still unsatisfied where the solve settled, which is both more accurate and something a user can act on without knowing what a Jacobian is.

Twenty-three sabotages. The important survivor was that **nothing asserted what "flush" means** -- every plane test checked the mate through `MateResiduals`, the same function the solver minimises, so reversing the convention reversed both and no test noticed. Two tests now assert the orientation against the geometry instead. A third survivor exposed a defect one layer down and out of scope here: `Transform.IsRigid` checks only that `Scale` is one and says nothing about the rotation's length, so it calls a transform rigid that is not -- and `Quatd.Rotate` documents that it assumes unit length. The test asserts the quaternion directly instead.

Three sabotages fail nothing and are recorded as such rather than papered over. Normalising the quaternion when a placement is read back cannot be observed, because every path that writes state normalises first and any non-unit input triggers a step that does; it stays because it decouples the mate residuals from the quaternion's length during differentiation, where the Jacobian nudges each component off the unit sphere on purpose. Ordering the solver's columns by body id is likewise not observable at any tolerance worth asserting -- permuting columns only changes pivot selection and hence last-bit rounding -- but it is what makes the system a function of the assembly rather than of the caller's argument order, which is what ADR-0011 asks for whether or not anyone can measure the difference. And applying the solution to grounded bodies as well as free ones changes nothing, because a grounded body's placement is read from its input either way; the narrower loop says what is meant.)*
- [x] **P5-T06** Component insertion, placement, and drag with live mate solving. *(The mate a document holds, and the crossing into the solver's vocabulary. Insertion and placement were already there -- `Assembly.WithDefinition`/`WithOccurrence` from P5-T04 -- so what this adds is `MateDefinition`, its storage and persistence, and `MateSystem`, which is the one place the two halves of an assembly meet. Above it there are components, occurrences and names; below it rigid bodies and residuals, and `OpenMCAD.Solver` has never heard of a document.

A mate end is **an occurrence path plus a name interpreted in the other document**. Not geometry, for the reason `ComponentDefinition.Source` is not a document: storing the plane a face lay on when the mate was made gives an assembly that quietly stops following its own parts. And not a `PersistentName` either, which is the subtler half -- a persistent name is a trail through *one* model's rebuild history, and a mate names something inside a component whose history this assembly has never seen. Turning it into geometry needs that document open, which is P5-T11's job, so it arrives through a delegate exactly as `DatumResolver`'s queries do.

`MateId` and `MateKind` are **the solver's own types, reused rather than mirrored**. A second identity would mean translating every diagnosis back before it could name anything a user recognises, and a second list of mate kinds would be two vocabularies nothing forces to agree -- the mistake `DocumentKind` had already made once here (P5-T04), found and removed one task earlier.

Three decisions the tests pin. **Deleting a placement deletes the mates that named it**, because a mate to something that is gone can neither be solved nor repaired -- there is nothing left to re-point it at -- so leaving it behind is leaving a permanent error as the price of one deletion. **A suppressed component takes its mates out of the solve**, or a component that is not in the product would go on constraining the ones that are. And **the dragged instance is the grounded one**: §5.9's live mate solving means the assembly follows the hand, not that the hand is corrected by the assembly, so a drag grounds what it holds and lets the rest come to it.

Sixteen tests and nineteen sabotages. Two survivors were the same gap in different clothes: only the *id* of an unresolved mate was asserted, never the reason, so swapping the two messages -- "the component is gone" against "the face is gone" -- failed nothing, though they are the two different repairs a user has to choose between. A third exposed something more interesting: the `try`/`catch` turning a bad mate into a `DocumentFormatException` is not doing what its comment claimed, because `DocumentCodec.Read` already converts an `ArgumentException` at the top. What it actually buys is a message naming which part of the file is wrong rather than "this document is damaged", and the test now asserts the message. The same is true of the occurrence check beside it, which had had the message assertion all along.

One sabotage fails nothing and is recorded rather than papered over: filtering grounded bodies out of `ApplyTo` is intent-revealing rather than load-bearing, since a grounded body's solved placement is its input placement and writing it back changes nothing.

**The drag session (later).** `AssemblyDragSession` carries over the two policies P4-T07 settled for sketches, because they are about the gap between how fast a pointer moves and how fast a solve finishes and that gap does not care what is being solved: **coalescing**, keeping only the newest position because every earlier one is stale before it could be worked on, and **a fixed baseline**, solving every frame from the assembly at mouse-down rather than from the last frame's answer.

An assembly forces a third that a sketch does not need: **only what the drag can disturb is handed to the solver.** The sketcher solves the whole sketch; a product is where the ten thousand components are. This is where `MateAnalysis.Groups` turned out to answer the wrong question -- it deliberately excludes grounded bodies, being about which bodies must be solved *together*, and a drag has just grounded the very body it must start from. `MateAnalysis.Reaching` is the right question: what does moving *this* disturb, following mates outward and **stopping at grounded bodies rather than passing through them**. Drag a bracket mated to a fixed base mated to a cover and the cover does not move, because the base cannot. The base is still in the solve -- it is what the dragged part is positioned against -- but the search does not expand from it, which is the difference between a two-body sub-problem and the whole product. Components outside the reachable set are not merely left unchanged: they are never passed to the solver, so no numerical wandering can move them, and `SolvedBodies` makes that measurable in a test rather than only in a frame time.

**There is no separate minimal-motion pass, and that is a claim rather than an omission.** The sketch solver needed one because it could jump to a distant solution; here every frame starts from the assembly at mouse-down and Levenberg-Marquardt converges to a nearby configuration, so "the answer closest to where things were" falls out of where the search begins. If a drag is ever seen to rearrange more than it must, that is the thing to build.

Sixteen sabotages, and three survived the first pass -- each because a fixture could not tell the difference, which is the more useful kind of survival. The baseline could not be shown to matter with a coincident chain, because a point pinned to a point is fully determined by where the dragged part is and returns to the same place whether frames chain or not; a **distance** mate leaves the direction free, and that is what creeps. The adjacency being two-way could only be shown by dragging the end the mate was *not* written from. And `Cancel` was asserted only on its return value, which let the session go on reporting the half-dragged assembly to anyone who asked for `Current` afterwards.)*
...
- [~] **P5-T11** Cross-document references: assembly → part, drawing → model, with external-reference tracking and out-of-date detection. *(The tracking, the out-of-date indicator, lock/break/unlock and cycle detection -- the four things §5.9 names. Resolving a reference into geometry is not here; see the end.

**Stored, not derived, and the reason is narrow.** Which documents an assembly uses *is* derivable from its components -- but what the target was when it was last read is recoverable from nothing, and neither is whether the user locked it. Those are exactly the two facts an out-of-date indicator needs, so this is a record rather than a view. It lives in `/refs/external.json`, the container part §5.8 reserved and `PackageContents` already had a slot for: a file browser or a PDM system asking "what does this need?" should not have to parse the graph, which for a large assembly is most of the cost of opening it. JSON and enum-by-name for the same reason -- this is the part most likely to be read by something that is not this program.

**Six health values, because each is a different thing to do.** The temptation is to collapse them and the trap is that `OutOfDate` and `HeldBehind` look alike and are opposites: one is a thing to fix, the other a thing the user chose, and merging them either nags about a deliberate freeze or hides that the freeze is now costing something. `Missing` outranks everything, because reporting a reference as current when the file it names has gone is the worst answer available. Unlocking deliberately **keeps the old stamp**: it says "follow this again", not "you are already current", so what was missed while frozen becomes work to do -- which is the whole point of unlocking. And `RefreshedTo` refuses a locked reference outright, since a rebuild that quietly refreshed one is how a user finds out their frozen drawing was not frozen.

Cycle detection returns **the path round the loop, trimmed of the road that reached it**, for the reason §5.6 gives about over-constrained sketches: a refusal the user cannot act on is barely better than no refusal. A document that cannot be opened is not evidence of a cycle -- refusing an edit on that basis would blame a file that might be innocent.

Twenty sabotages. Three survived the first pass and each was a different kind of finding. Clearing the stamp on unlock failed nothing, because a cleared stamp also reports `OutOfDate` against a *moved* target -- only asking about the unmoved one tells them apart. The visit memo failed nothing because it is pure cost, not correctness: every answer stays right and the search merely goes exponential where parts share a library, so it is pinned by counting how many times a document is asked about rather than by timing anything. The third is the interesting one: **the memo and taking a document back off the path each mask the other's removal**, and removing both produces a false cycle on an ordinary diamond. Both stay, and a sabotage now covers the pair.

**What is left: resolution.** Nothing yet turns a target into an open document or an element name into geometry -- `ComponentDefinition.Source`, `MateEnd.Element` and the delegates in `MateSystem` and `DatumResolver` are all still stand-ins for it. That needs a document store, which is P5-T12's session management, so the two want doing together.)*
- [~] **P5-T12** File open/save/new/recent UI; document session management; dirty tracking. *(Dirty tracking and the save/open wiring; the UI is not here and waits for the shell, the same split P4-T15 made for the sketch editor and for the same reason -- ADR-0007's view-model layer is meant to be built against a real requirement before the chrome that hosts it exists (P6-T01/T02).

**`IsDirty` compares by reference, and that is the interesting decision.** An immutable document makes undo a matter of holding an earlier reference (§5.4), so a user who edits and then undoes back to where they started is holding the very document that was saved -- and reporting that as dirty would ask them to save a file that cannot have changed. Where reference and value comparison differ is a document edited into an *equal* state by a different route: add a dependency, remove it again, and this says dirty. That is the safe direction to be wrong in, because offering to save something that need not be saved costs a keystroke and the other way costs the work. Both cases have tests, and the second one exists because a sabotage swapping `ReferenceEquals` for `Matches` failed nothing without it.

**The save wiring closed a hazard `persistence.md` had just named.** `/refs/external.json` was a caller-supplied opaque part like the thumbnail beside it, and P5-T11's own note said a save command that forgot to compose it would lose the stamps. It is now written from the document and folded back into it on open, so a caller cannot forget: `PackageContents.ExternalReferences` is what was on disk, and the document is what is true. This is the one part of the container that is not the caller's business, and the asymmetry is deliberate -- the stamps are the only record of what each dependency was when it was last read, and losing them reports every dependency as current for ever after.

That change was caught by an existing test rather than by a new one: `TheOtherPartsOfTheLayoutAreKept` handed the part in and expected it back, which is no longer the contract. It now asserts the opposite and says why.

Ten sabotages. Two survived and both were decisions no test could see. `MarkSaved`'s assignment turned out to resist sabotage entirely -- self-assignment is CS1717 and dropping it makes the field readonly-able, so the toolchain rejects every form of it; assigning a *different* document is the only expressible version and that is caught.

**The store, and the resolution it unblocks.** `IDocumentStore` turns a target into an open document, and `ComponentElements` turns a mate's element name into geometry by opening the component it names. Between them they replace the last of the cross-document stand-ins: `MateSystem.For` has taken an `elementOf` delegate since it was written and every test of it so far supplied a fake, so `AMateResolvedFromDiskActuallySolves` is the first time the whole path runs -- a mate naming a datum in another document, that document read off disk, the geometry found, the assembly solved.

**An interface, because the answer is not always a file.** A component can come from a folder, a vault, a library shipped with the application, or a fixture that never touches a disk. Everything above works in terms of a target string it does not interpret, which is what lets those be implementations rather than special cases.

**`StampOf` is separate from `Open` on purpose.** An out-of-date indicator asks about every dependency of an assembly, and answering by opening each one would make drawing the indicator as expensive as loading the product -- the cost §5.9's lightweight display modes exist to avoid. So the stamp is the modification time and the length, and what that costs is written down rather than hidden: two edits inside the filesystem's timestamp granularity that leave the length unchanged are indistinguishable. A vault-backed store should use the revision the vault already knows, which is both cheaper and exact.

**Reference geometry only, and deliberately.** A datum has a name that survives the component being edited, which is what a durable mate needs; a *face* does not -- naming one means a `PersistentName`, a trail through that component's own rebuild history that can only be resolved by replaying it. So mating to faces is not yet possible and says so rather than half-working. A coordinate system is refused for a different reason: it could mean its XY plane or its origin, the two readings `DatumResolver` separates by asking what *role* an input plays, and a mate element has no role -- it simply is.

**Two findings, both from the sabotage pass, and one was a real hole.** The fence against escaping the root was a bare `StartsWith`, which passes a sibling folder whose name merely starts with the root's: a store rooted at `parts` would happily open through `../parts-archive`. Comparing against the root plus a separator fixes it, and the sabotage that removes the separator now fails a test written for exactly that shape. The second finding was three overlapping existence checks -- one in `Locate`, one in `StampOfFile`, and the `IOException` catch below it -- where any one alone gave the right answer, so two of them could be removed with nothing failing. `Locate`'s is gone; it made that method mean two things, and existence is now `StampOfFile`'s single answer.

**One sabotage survives, knowingly.** Removing `StampOfFile`'s existence test still returns null for an absent file, because reading the length throws `FileNotFoundException` and the catch below answers identically. The overlap is the ordinary path plus the race -- the file going away between the test and the read -- and the race cannot be produced from a test. Recorded rather than papered over, and the comment there now says so. Eleven of twelve sabotages failed a test; four of those tests exist because the first pass showed the guard unprotected.

**What is left: the UI.** File open/save/new/recent as chrome, which waits for the shell (P6-T01/T02) on the same reasoning as the dirty-tracking half above.)*
