...
- [ ] **P2-T02** `HwndHost`-based viewport control; per-monitor DPI v2; resize, occlusion, device-loss recovery. *(Done except recovery. The viewport is live in the window, sizes itself correctly under per-monitor DPI — verified on a 150% display — and stops rendering when the window is hidden, which `CompositionTarget.Rendering` gives for free. Device loss is detected and reported but not recovered from: the device, swapchain and every pass would have to be rebuilt, and until that exists the viewport stops drawing rather than spinning on a dead device.)*
...
- [ ] **P2-T08** Camera and navigation: orbit, pan, zoom, zoom-to-fit, zoom-to-selection, standard views, view cube, perspective/orthographic toggle, configurable mouse profiles. *(Everything but the view cube. Mouse profiles are a rebindable table with presets modelled on SolidWorks, Fusion and Onshape, because users arrive with a decade of muscle memory from whichever package they came from; modifiers match exactly, so a modified drag cannot silently fall back to the unmodified gesture. Rates scale with the viewport height on both axes, and the wheel zooms towards the pointer. Zoom-to-selection is `ZoomToFit` over the selection's bounds and lands with P2-T09's geometry. The view cube needs overlay rendering and belongs with P2-T11. Two of the direction tests were written against points on the silhouette, which are turning points of the projection — they passed with the drag deliberately inverted until they were rewritten.)*
...
- [ ] **P2-T11** Grid, origin triad, reference plane display, background gradient/environment. *(Gradient and grid done; the triad and reference planes are not. The grid is computed per pixel from a ray-plane intersection rather than drawn as line geometry: line geometry needs an extent and a spacing fixed in advance, and a CAD user zooms across six orders of magnitude in a session, so the lines either run out at the edge of the world or collapse into a solid mass. Screen-space derivatives keep every line the same width at any distance. Spacing snaps to a power of ten below a tenth of the scene, so it lands on a number a user can count in, and is taken from the scene rather than the camera because a reference that changes density as you zoom is worse than one at the wrong scale. One full-screen triangle, no vertex buffer, no depth writes. The origin triad and the view cube P2-T08 still owes are the same piece of work — a screen-space overlay — and are next.)*
