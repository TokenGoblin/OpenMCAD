...
- [ ] **P2-T12** MSAA, FXAA, optional SSAO; a defensible default material and lighting setup. *(MSAA done at four samples, negotiated with the device and falling back through two to one. The whole scene is drawn multisampled and resolved into the back buffer, rather than filtered afterwards: the aliasing that matters in CAD is on geometric silhouettes, and a post-process works from the finished image and can only guess where an edge was — softening text and fine detail while never quite fixing the staircase. The ID buffer is deliberately left single-sampled, since resolving indices would average them into a number naming an entity that is under the cursor nowhere. FXAA is therefore unlikely to be worth adding; SSAO and a considered default material are still open, and the lighting is the offset headlight from P2-T05.)*
